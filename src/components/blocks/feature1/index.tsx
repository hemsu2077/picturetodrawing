...
export default function Feature1({ section }: { section: SectionType }) {
  ...
  const mediaSrc = section.image?.src;
  const baseMediaSrc = mediaSrc?.split("?")[0] ?? "";
  const isVideo = baseMediaSrc ? /\.(mp4|webm|ogg|mov|m4v)$/i.test(baseMediaSrc) : false;
  const mediaClassName = [
    "w-[450px] justify-self-center rounded-lg object-cover aspect-square",
    section.image?.className,
  ]
    .filter(Boolean)
    .join(" ");
  const altText = section.image?.alt || section.title || "";
    ...
    <section id={section.name} className="py-16 bg-background">
      <div className="container max-w-6xl mx-auto px-6">
        <div className="grid items-center gap-16 lg:grid-cols-2 lg:gap-20 animate-in fade-in duration-700 slide-in-from-bottom-4">
          {section.image && mediaSrc && (
            <div className="relative order-2 lg:order-1 group">
              <div className="pointer-events-none absolute -inset-4 -z-10 bg-gradient-to-r from-background via-muted/20 to-background rounded-3xl blur-xl opacity-50 transition-opacity duration-500 group-hover:opacity-70"></div>
              {isVideo ? (
                <video
                  src={mediaSrc}
                  autoPlay
              ...
              <div className="pointer-events-none absolute -z-20 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full bg-accent/5 blur-3xl rounded-full transition-all duration-500 group-hover:bg-accent/10"></div>
            </div>
          )}
